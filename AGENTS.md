...
- **Finding relationships**: `query_graph_tool` with callers\_of/callees\_of/imports\_of/tests\_for
...
4. Use `query_graph_tool` pattern="tests\_for" to check coverage.
...
给你的参考数据，若你需要用到头像、icon图标资源，就将其上传到oss上，你可以用已有的web直传方案——/post\_signature，prefix由你决定，格式通常: {prefix}/{yyyy/MM/dd}/{uuid}。
...
本项目先不关注移动端业务。
支付宝电脑网站支付沙箱环境，买家账号：<fyfmja1112@sandbox.com>，登录/支付密码：111111，用户UID2088722101655074；商户账号<iyjxud1074@sandbox.com>，登录密码111111，商户PID2088721101655066。
...
- 开发过程中要有精确的注释信息，在设计实体类的时候属性上要有注释说明，在业务流程中尤其关键代码开始结束位置，以中文注释为主可以包含英文词。
