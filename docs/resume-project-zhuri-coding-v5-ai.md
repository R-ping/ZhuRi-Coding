# 逐日 Coding · 简历版项目经历 v5（AI 版·包含关系，可直接粘贴）

> 版本 v5（2026-09-10）：在 v4（Java 4 + AI 3 = 7 条）基础上，纳入 **AI 内容治理与诚信（AIGC 检测 + 申诉闭环）、语义搜索与推荐、AI 商业化工程**三块新落地的源码能力。
>
> **用户拍板口径（2026-09-10）**：
>
> - 采用方案 B 结构：**Java 4 条稳定盘 + AI 条**；AI-3（UGC 治理）与 AI-4（AIGC 检测）**合并为一条「AI 内容治理与诚信体系」**；
> - 合并后主推 **AI 5 + Java 4 = 9 条**；若版面允许凑 10 条，见文末「可选第 10 条」（把"单篇问答 + AI 速览"从 AI-1 拆出）；
> - 措辞尺度：新链路一律写「**设计并实现**」，**不写"已上线创收/已产生收入"**——AI 额度包支付链路代码完整但沙箱回调未真实跑通（需外网可达），统一 AI 入口未做；
> - 面试主线不变：**AI-1（检索质量）→ AI-3（治理与诚信）→ AI-5（商业化工程）**；Java 岗前置 J-1/J-3。
>
> 相对 v4 的变化：AI 侧 3 → 5 条（新增治理诚信合并条、语义搜索与推荐条、商业化工程条）；Java 侧保持 4 条，新增横向"异步任务可靠性与补偿体系"转为**口述补充**（不占版面）；技术栈补 pgvector 复用面、SSE、支付宝沙箱额度包。
>
> ⚠️ 待定稿项（沿用 v4，文档不替你做决定）：
>
...
>
> 📌 **v6 已发布**：本文件为 v5 存档；最新版见 `docs/resume-project-zhuri-coding-v6-ai.md`（补入父子分块、BM25+向量 RRF 混合召回、语义缓存，并把社交登录补入 J-4）。
...
1. **自研 RAG 社区问答 + 单篇问答（检索质量与成本工程）**：文章经 Embedding 入 pgvector(1024)；口语问题先 Query Rewrite 再宽召回 15 候选 → LLM Rerank 精排到 TopK → 答案逐句标注 `[n]` 引用、前端可点击溯源原文；SSE 逐字流式 + 最近 6 轮会话记忆滑窗解析指代；另做"只答本文"的文章级单篇问答与 AI 摘要，摘要 Redis 缓存 24h、长文截断 1.2 万字符控 token 成本——同一 LLM 基座按"多篇检索 / 单篇收敛"两种上下文形态提供能力。
...
5. **AI 商业化工程（可衡量 → 可变现 → 可复用）**：**可衡量**——问答 👍/👎 反馈闭环（幂等落库）+ 12 条真实黄金问答对的 recall@5/10 离线评测集（检索改动前后回归基线）+ 功能级调用指标与用户兴趣画像注入；**可变现**——每日免费配额（Redis 自然日计数、异常 fail-open）+ AI 额度包钱包（支付宝回调按订单号前缀分发、金额以服务端为准防篡改、`UPDATE … WHERE balance>0` 原子扣减防超扣、消费优先于免费额度）；**可复用**——发布预检产出的摘要/标签已回填文章元数据，进一步复用为公开 AI 速读页（SEO 入口）与详情页 AI 摘要卡。
...
1. **自研 RAG 问答 + AI 速览**：文章向量化入 pgvector，Query Rewrite → 宽召回 + LLM Rerank → 回答带 `[n]` 引用溯源、SSE 流式 + 多轮记忆；摘要 Redis 缓存 24h 控成本；另支持"只答本文"单篇问答。
...
| #    | 面试可展开的证据点 | 对应代码位置 |
| ---- | ---- | ---- |
| AI-1 | Query Rewrite→宽召回 15→LLM Rerank→`[n]` 引用；SSE 事件 delta/done/error；memory 滑窗 ≤12 条；摘要 key `ai:article:summary:{id}` TTL 24h、正文截断 1.2 万字符 | `AiAskServiceImpl`、`AiAskController.askStream`、`ArticleQaServiceImpl` |
...
| AI-5 | 配额：`ai:quota:daily:{userId}:{date}`，首计设当日过期、Redis 异常 fail-open、DAILY_QUOTA=20；额度包：orderNo=`ai`+雪花作 out_trade_no，`PayController.notify` 前缀分发（不动课程逻辑）、金额以订单分为准、`WHERE balance>0` 原子扣、入账幂等；反馈：`ap_ai_feedback` 唯一键 user+feature+scene+question_hash；评测：`ai-eval/eval-questions.json` 12 条真实 golden + recall@5/10；画像：收藏 30 天聚合标签 Top5 注入 Prompt（无关可忽略）；路由：`AiModelRouter` 全 Bean 注册 + feature 映射（当前单模型恒默认，第二模型零改动接入） | `AiQuotaServiceImpl`、`AiTopupServiceImpl`、`PayController`、`AiFeedbackServiceImpl`、`AiEvalServiceImpl`、`UserInterestServiceImpl`、`AiModelRouter` |
...
> 面试主战线：**AI-1（检索质量）→ AI-3（治理与诚信）→ AI-5（商业化工程）→ J-3（支付兜底）**；AI 岗前置 AI 1~5，Java 岗前置 J-1/J-3。
...
把 AI-1 中的"单篇问答 + AI 速览"拆出为独立条目，凑 **AI 6 + Java 4 = 10 条**：

> **AI 速览与文章级问答（上下文收敛）**：同一 LLM 基座提供"只答本文"的文章级问答与 AI 摘要——SSR 详情页直出摘要卡（Redis 24h 缓存、正文截断 1.2 万字符），问答限定"依据仅来自本文"并复用 SSE 流式；另由本文生成"读完想问"问题集（≤30 字、点击即进浮层自动提问），把 AI 入口从全局收敛到文章级。

拆出后 AI-1 相应收窄为"RAG 社区问答（多篇检索）"。
