 ...
 * 沸点评论异步审核可靠队列表实体
 *
 * 与 {@link ApCommentAuditTask}（文章评论）同构但独立建表：
 * 两张源表 ap_comment / ap_pins_comment 的 id 各自 AUTO 自增会撞号，
 * 无法共用同一 comment_id 唯一键，故沸点评论治理独立一套队列。
 * 解决【先展示后审核】窗口期：评论发布时立即落库（用户可见），同时持久化一条待审核任务；
 * 服务重启导致进程内异步任务丢失时，由定时补偿任务（PinsCommentAuditRecoveryTask）重新拉起。
 */
@Data
@TableName("ap_pins_comment_audit_task")
public class ApPinsCommentAuditTask implements Serializable {
